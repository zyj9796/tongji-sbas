function [data, count] = freadbkB(infile, lines, bkformat, r0, rN, c0, cN)
...
if (nargin <  7)
    cN = 0; end;%				fall through (not...)
if (nargin <  6) 
    c0 = 0; end;%				fall through
if (nargin <  5) 
    rN = 0; end;%				fall through 
if (nargin <  4) 
    r0 = 0; readwholefile=true; end;%	fall through

if (nargin <  3) 
    bkformat='float32'; end;%		fall through
if (nargin <  2) 
    lines=1; end;%				fall through
if (nargin <  1)
...
if (readwholefile==true)
  ...
  if (cN > filewidth)   
      error('cN > width'); end;
  if (~isint(filewidth))
      error('numlines file seems to be wrong(?)'); end;
